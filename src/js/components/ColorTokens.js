...
class ColorTokens extends HTMLElement {
  /**
   * @description Initializes the ColorTokens component, sets up the shadow DOM, and prepares for rendering. This constructor does not take any parameters and is called when the component is instantiated.
   */
  constructor() {
    super();
  }
  ...
  /**
   * @description Called when the component is added to the DOM. It renders the component, subscribes to changes in the working palette from the store, adds event listeners for user interactions, and updates the displayed tokens based on the current state.
   */
  connectedCallback() {
    this.render();
  ...
  /**
   *
   * @param {Event} event - The event object representing the user interaction. This method handles 'change' events to update the tokens and 'click' events to trigger the download of the generated tokens.
   */
  handleEvent(event) {
    if (event.type === 'change') {
      this.update();
    }
    if (event.type === 'click' && event.target.id === 'download-tokens') {
  ...
  /**
   * @description Adds event listeners for 'change' and 'click' events to the component, allowing it to respond to user interactions such as changing token formats or downloading tokens.
   */
  addEventListeners() {
  ...
  /**
   *  @description Updates the displayed color tokens based on the current working palette, selected token format, color space, and color scheme. It generates the appropriate token code and updates the content of the token container in the component.
   *
   */
  update() {
    const { workingPalette } = store.getState();
  ...
  /**
   * @description Renders the HTML structure of the ColorTokens component, including form controls for selecting token formats, color spaces, and color schemes. It also includes buttons for copying and downloading the generated tokens.
   */
  render() {
    this.innerHTML += `
      <div class="corn-row">
  ...
  /**
   * @description Cleans up event listeners and unsubscribes from the store when the component is removed from the DOM.
   */
